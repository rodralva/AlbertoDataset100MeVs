...
def file_globals(rootfile,file_type=""):
    if rootfile.__contains__("OLD"):
       file = uproot.open(rootfile)["hnlpizeroana"]
    else:
       file = uproot.open(rootfile)["hnlana"]
    events  = file["events"]
    ...
    if rootfile.__contains__("pyPROCCESSED"):
        N_SLICES = len((events["slc_n_trks"].array()))
    else:
    ...
    if file_type=="hnls": 
        if rootfile.__contains__("pyPROCCESSED"):
            ev_comp=          (events['slc_comp']           .array() )>0.5
        else:
            ev_comp=ak.flatten(events['slc_comp']           .array() )>0.5
            
        nSig    =((types==0)*ev_comp)
        nNonFV  =((types==1)*ev_comp)
        nNonFV  =((types>0)*ev_comp)

        globals_file["CUTS"] =np.array(nSig+nNonFV)
